...
export const TimeToBeat = {
  accessor: ({
    ...
    if (timeToBeat?.hltb_id) {
      const minutesToBeatValues = [
        ...(timeToBeat.minutes_to_beat_completionist > 0
          ? [timeToBeat.minutes_to_beat_completionist]
          : []),
        ...(timeToBeat.minutes_to_beat_extras > 0
          ? [timeToBeat.minutes_to_beat_extras]
          : []),
        ...(timeToBeat.minutes_to_beat_main_game > 0
          ? [timeToBeat.minutes_to_beat_main_game]
          : []),
      ];

      const hoursToBeatMin = minutesToHours(
        Math.min(...minutesToBeatValues),
        1,
      );
      const hoursToBeatMax = minutesToHours(
        Math.max(...minutesToBeatValues),
        1,
      );
  ...
  sortType: (
    ...
    (timeToBeat?.hoursToBeatMin || 0) > (nextTimeToBeat?.hoursToBeatMin || 0)
      ? -1
      : 1,
};
...
const TimeToBeatCellValue = ({
  value: { hltbId, hoursToBeatMax, hoursToBeatMin },
  ...
  return (
    <a href={`https://howlongtobeat.com/game.php?id=${hltbId}`}>
      min: {hoursToBeatMin} hours
      <br />
      max: {hoursToBeatMax} hours
    </a>
  );
};
