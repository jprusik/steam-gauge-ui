...
export const LibrarySummary = ({
  accountData: { steamid, timecreated },
  detailsLoading,
  libraryApps = [],
  ...
  const { installSize, selectionCount, timePlayed, usdSum } =
  ...
  return (
    <Fragment>
      <SummaryText>
        ...
        {timePlayed} have been spent playing this library, which includes{" "}
        {selectionCount} items
        {!detailsLoading &&
          `, is valued at ${usdSum}, and requires ${installSize} of disk space`}
      </SummaryText>
      <SecondaryItems>
        <ShareBar
        ...
        />
        <SteamReceiptLink to="/receipt" className="bookmarklet-link">
...
function summaryData(libraryApps: AccountApp[]): SummaryData {
  ...
  const usdSum = roundToPlaces(
    numberValueSum(
      libraryApps,
      ({ store_price_default_usd }: { store_price_default_usd: number }) =>
